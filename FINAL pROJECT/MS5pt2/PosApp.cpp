...
void PosApp::POS() {
        ...
        if (cin.fail()) {
            ...
            if (item != nullptr) {
                if (item->quantity() > 0) {
                    ...
                    // Place the dynamic cast here
                    if (Perishable* p = dynamic_cast<Perishable*>(item)) {
                        p->displayInPOS();
                    } else {
                        item->displayInPOS();
                    }
    ...
    // Determine maximum width of first column
    int max_name_width = 0;
    for (Item* addedItem : addedItems) {
        int name_width = strlen(addedItem->getname());
    ...
    for (Item* addedItem : addedItems) {
        cout << "| " << left << setw(18) << addedItem->getname() << "  | " << right << setw(9) << addedItem->cost() << " |";
        if (addedItem->isTaxed()) {
            cout << "  T  |" << endl;
        } else {
            cout << "     |" << endl;
        }
    ...
    cout << "+---------------------^-----------^-----+" << endl;
    cout << "| total: " << setw(28) << fixed << setprecision(2) << total << " |" << endl;
    cout << "^---------------------------------^" << endl;
}
